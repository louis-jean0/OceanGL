...
//     // Specular
//     vec3 viewDir = normalize(viewPosition - pos);
//     vec3 reflectDir = reflect(-viewDir, normal); // Adjusted to use viewDir instead of lightDir
//     vec3 halfwayDir = normalize(lightDir + viewDir);
//     float spec = pow(max(dot(normal, halfwayDir), 0.0), 32);
//     vec3 specular = specularStrength * spec * lightColor;
...
//     vec3 finalColor = ambient + diffuse + specular + fresnelColor;
//     finalColor = mix(baseColor, finalColor, 0.7);
//     finalColor = mix(finalColor, skyboxColor, fresnelFactor * 0.5); // Adjust the blend factor for the skybox reflection
